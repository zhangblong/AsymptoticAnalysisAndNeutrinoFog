...
# When the threshold is large enough, there should be less species of neutrinos considered.
# However, the MaxNuRecoilEnergies still provide the complete set, which would cause a error in the Effine causing
# the negative rate.
# Therefore, we also add a new output in the GetNuFluxes for conveniences and the Flux_norm in the Bin....
...
def BinnedNeutrinoRates2(E_th,E_max,ne,Nuc,Flux_norm=NuFlux):
    E_be = logspace(log10(E_th),log10(E_max),ne+1)
    #E_r_max = MaxNuRecoilEnergies(Nuc)
    # Load neutrinos
    Names,Solar,E_nu_all,Flux_all,Flux_norm_default,errs,nuMaxEnergy = GetNuFluxesCorrect(E_th,Nuc)
