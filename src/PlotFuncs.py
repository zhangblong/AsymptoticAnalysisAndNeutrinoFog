...
def SILimits2023(ax,Annotations=True,facecolor=[0.0, 0.62, 0.38],edgecolor='darkgreen',alph=1):
    ymax = ax.get_ylim()[1]
    plt.sca(ax)
    pek = line_background(5,'k')
    ...
    dat = loadtxt('../data/WIMPLimits/SI/DEAP-3600.txt')
    plt.plot(dat[:,0], dat[:,1],color='#4ff09d',linewidth=3)
...
def MakeLimitPlot_SI2023(Annotations=True,Collected=False,\
                     xmin=0.1,xmax=1.0e4,ymax=1.0e-36,ymin=1.0e-51,\
                     facecolor=[0.0, 0.62, 0.38],edgecolor='darkgreen',edgecolor_collected='darkgray',\
                     alph=0.5,lfs=35,tfs=25,\
                     xlab=r"DM mass [GeV$/c^2$]",ylab=r"SI DM-nucleon cross section [cm$^2$]"):
    ...
    if Collected:
        ...
        if Annotations:
            plt.text(21.,3.0e-42,r"{\bf CDMSlite}",color="blue",fontsize=22,rotation=0)
            plt.text(2.65,3e-42,r"{\bf DarkSide}",color="green",fontsize=22,rotation=0,ha='right')
            plt.text(850.0,2.4e-45,r"{\bf PandaX-II}",color="navy",fontsize=22,rotation=21)
            plt.text(2.0,1e-44,r"{\bf XENON1T}",color="darkgreen",fontsize=22,rotation=-70)
            plt.text(6.5,4.0e-44,r"{\bf EDELWEISS}",color=[0.67, 0.31, 0.32],fontsize=18,rotation=-25)
            plt.text(2000.0,5.4e-43,r"{\bf PICO60}",color=[0.5, 0.0, 0.13],fontsize=22,rotation=22)
            plt.text(1.5e3,3.5e-44,r'{\bf DEAP-3600}',color='#4ff09d',fontsize=22,rotation=23)
            plt.text(4000.0,8.7e-45,r"{\bf LUX}",color="crimson",fontsize=21,rotation=19)
            plt.text(1800,4e-46,r"{\bf PandaX4T}",color="orange",fontsize=22,rotation=26)
            plt.text(250.0,6e-47,r"{\bf XENONnT}",color="c",fontsize=22,rotation=26)
        
        AllLimits = loadtxt("../data/WIMPLimits/SI/AllLimits-2023.txt")
        plt.fill_between(AllLimits[:,0], AllLimits[:,1],edgecolor=None,y2=ymax,facecolor=facecolor,alpha=alph,zorder=0)
        SILimits2023(ax,facecolor=facecolor,edgecolor=edgecolor,alph=alph)
