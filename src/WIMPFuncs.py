...
# When the threshold is large enough, the maximal energy would be less than it.
def BinnedWIMPRate2(E_th,E_max,ne,m_vals,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs):
    EmaxSet = np.array(list(map(lambda m: MaxWIMPEnergy(Nuc.MassNumber,m,**kwargs),m_vals)))
    massSet= m_vals[EmaxSet>(2*E_th)]
    E_be = logspace(log10(E_th),log10(E_max),ne+1)
    nm = size(massSet)
    R = zeros(shape=(nm,ne))
    for i in range(0,nm):
        E_r_max = MaxWIMPEnergy(Nuc.MassNumber,massSet[i],**kwargs)
        Efine = logspace(log10(E_th),log10(E_r_max),1000)
        R_tot = trapz(dRdE(Efine,massSet[i],1.0e-45,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs),Efine)
        #print(R_tot)
        dR = dRdE(E_be,massSet[i],1.0e-45,Nuc,NuclearEnhancementFactor,FormFactor,gvmin,**kwargs)
        R[i,:] = 0.5*(E_be[1:]-E_be[0:-1])*(dR[1:]+dR[0:-1])
        R[i,:] = R_tot*R[i,:]/sum(R[i,:])
    return [massSet,R]
